...
def build_model(model_cfg: DictConfig) -> BasicModel:
    ...
    neck = hydra.utils.instantiate(
        model_cfg.neck, in_channels_list=backbone.out_channels_list[-n:]
    )

    heads = nn.ModuleDict()
    for head_name, head_cfg in model_cfg.heads.items():
        heads[head_name] = hydra.utils.instantiate(
            head_cfg, in_channels=neck.out_channels
        )
...
def _create_param_groups(model: nn.Module):
    ...
    assert num_total_params == num_total_params_groups, (
        num_total_params,
        num_total_params_groups,
    )

    param_groups = [{"params": wd_params}, {"params": no_wd_params, "weight_decay": 0}]
    return param_groups
