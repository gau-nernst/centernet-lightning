...
class CenterNetLoss(nn.Module):
    def __init__(
        ...
        box_loss: nn.Module,
        radius_function: Callable[[float, float], Tuple[float, float]],
    ):
        super().__init__()
    ...
    def calculate_centers_radii(
        self, boxes: Iterable[_Box], stride: int = 4
    ...
    def update_heatmap(
        ...
        for (cx, cy), (rx, ry), label in zip(centers_int, radii_int, labels):
            ...
            gaussian = grid_x.square() / (2 * std_x**2) + grid_y.square() / (
                2 * std_y**2
            )
            gaussian = torch.exp(-gaussian)
            gaussian[
                gaussian < torch.finfo(gaussian.dtype).eps * torch.max(gaussian)
            ] = 0
    ...
    def forward(
        self, outputs: Dict[str, torch.Tensor], targets: Dict[str, Any], stride: int = 4
    ):
        ...
        box_loss = torch.tensor(0.0, dtype=dtype, device=device)
        for i, (boxes, labels) in enumerate(zip(target_boxes, target_labels)):
            ...
            box_samples, indices = self.sample_nxn_boxes(
                boxes, centers_int, out_w, out_h
            )
            pred_boxes = self.decoder.gather_and_decode_boxes(
                box_offsets[i], torch.tensor(indices, device=device)
            )
            box_loss = box_loss + self.box_loss(
                pred_boxes, torch.tensor(box_samples, device=device)
            )
