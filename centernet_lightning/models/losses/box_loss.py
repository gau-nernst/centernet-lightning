...
__all__ = ["L1Loss", "SmoothL1Loss", "IoULoss", "GIoULoss", "DIoULoss", "CIoULoss"]
...
def _get_iou(boxes1: torch.Tensor, boxes2: torch.Tensor):
    ...
    intersection = torch.clamp_min(x2_inter - x1_inter, 0) * torch.clamp_min(
        y2_inter - y1_inter, 0
    )
...
class DIoULoss(nn.Module):
    ...
    def forward(self, boxes1: torch.Tensor, boxes2: torch.Tensor, eps=1e-8):
        ...
        distance_sq = (centers2[..., 0] - centers1[..., 0]).square() + (
            centers2[..., 1] - centers1[..., 1]
        ).square()
...
class CIoULoss(nn.Module):
    ...
    def forward(self, boxes1: torch.Tensor, boxes2: torch.Tensor, eps=1e-8):
        ...
        angle_diff = (
            (torch.atan(w1 / (h1 + eps)) - torch.atan(w2 / (h2 + eps))) * 2 / math.pi
        )  # 2 / pi = 90 degree
        v = angle_diff.square()
        alpha = v / (1 - iou + v + eps)
