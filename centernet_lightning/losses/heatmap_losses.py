...
class CornerNetFocalLoss(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor):
        ...
        neg_weight = torch.pow(1-targets, self.beta)    # when target = 1, this will be 0
        ...
        pos_loss = -torch.pow(1-probs, self.alpha) * F.logsigmoid(inputs) * pos_weight
        neg_loss = -torch.pow(probs, self.alpha) * F.logsigmoid(-inputs) * neg_weight
        ...
        if self.reduction == 'mean':
            loss = loss / torch.sum(pos_weight) 
